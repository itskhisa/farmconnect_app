#!/usr/bin/env python3
"""
Complete rewrite of storage.dart with all helpers inside StorageService class.
This is the definitive version — no more patching.
"""
import os
...
// ── Supabase config (top-level constants) ─────────────────────
const _kSupabaseUrl = 'https://jiplkqrczvjliriqwjzd.supabase.co';
const _kSupabaseKey =
...
class StorageService {
  ...
  Map<String, String> get _headers => {
    'apikey': _kSupabaseKey,
    'Authorization': 'Bearer $_kSupabaseKey',
  ...
  Future<List<Map<String, dynamic>>> _sbSelect(String table,
      {String? filter}) async {
    final url =
        '$_kSupabaseUrl/rest/v1/$table${filter != null ? '?$filter' : ''}';
    try {
      final r = await http
  ...
  Future<bool> _sbUpsert(String table, Map<String, dynamic> data) async {
    try {
      final r = await http
          .post(
            Uri.parse('$_kSupabaseUrl/rest/v1/$table'),
            headers: {
              ..._headers,
  ...
  Future<bool> _sbDelete(String table, String id) async {
    try {
      final r = await http
          .delete(
            Uri.parse('$_kSupabaseUrl/rest/v1/$table?id=eq.$id'),
            headers: _headers,
          )
          .timeout(const Duration(seconds: 15));
      return r.statusCode >= 200 && r.statusCode < 300;
  ...
  Future<String?> registerUser(
    ...
    final existing =
        await _sbSelect('users', filter: 'phone=eq.$clean&select=id');
  ...
  Future<void> fetchAndCacheCounties() async {
    ...
    try {
      final rows =
          await _sbSelect('users', filter: 'id=eq.$uid&select=counties');
      if (rows.isNotEmpty) {
  ...
  Future<List<CropModel>> fetchCrops() async {
    ...
    if (rows.isEmpty &&
        await _sbSelect('users', filter: 'id=eq.$uid&select=id')
            .then((r) => r.isEmpty)) {
      return _getCachedCrops();
    }
    final crops = rows
        .map((r) => CropModel.fromJson({
              'id': r['id'],
              'name': r['name'],
              'variety': r['variety'] ?? '',
              'emoji': r['emoji'] ?? '🌱',
    ...
    await prefs.setString(
        'crops_$uid', jsonEncode(crops.map((c) => c.toJson()).toList()));
    return crops;
  }
  ...
  Future<void> addCrop(CropModel crop) async {
    ...
    await _sbUpsert('crops', {
      ...
      'expected_days': crop.expectedDays,
  ...
  Future<void> updateCrop(CropModel updated) async {
    ...
    await _sbUpdate('crops', updated.id,
        {'name': updated.name, 'variety': updated.variety, 'status': updated.status, 'area': updated.area});
  }
  ...
  Future<void> deleteCrop(String id) async {
    ...
    await prefs.setString(
        'tasks_$uid', jsonEncode(tasks.map((t) => t.toJson()).toList()));
    await http.delete(
      Uri.parse(
          '$_kSupabaseUrl/rest/v1/tasks?user_id=eq.$uid&crop_id=eq.$id'),
      headers: _headers,
  ...
  Future<List<LivestockModel>> fetchLivestock() async {
    ...
    final lst = rows
        ...
        .toList();
    await prefs.setString(
        'livestock_$uid', jsonEncode(lst.map((l) => l.toJson()).toList()));
    return lst;
  }
  ...
  List<LivestockModel> _getCachedLivestock() {
    ...
    if (raw == null) return [];
    return (jsonDecode(raw) as List)
        .map((j) => LivestockModel.fromJson(j))
        .toList();
  }
  ...
  Future<void> addLivestock(LivestockModel l) async {
    ...
    await prefs.setString(
        'livestock_$uid', jsonEncode(lst.map((x) => x.toJson()).toList()));
  ...
  Future<void> deleteLivestock(String id) async {
    ...
    final vaccRecords = getVaccinationRecords()
        .where((r) => r['livestockId']?.toString() != id)
        .toList();
    await _prefs?.setString('vaccination_records', jsonEncode(vaccRecords));
    final tasks =
        _getCachedTasks().where((t) => t.cropId != id).toList();
    await prefs.setString(
        'tasks_$uid', jsonEncode(tasks.map((t) => t.toJson()).toList()));
  }

  // ── Vaccination records (local) ────────────────────────────────
  ...
  Future<void> addVaccinationRecord(Map<String, dynamic> record) async {
    ...
    if (record['nextDueDate'] != null &&
      ...
      await addTask(TaskModel(
        id: 'vacc_${record['id']}',
        title:
            'Vaccinate ${record['animalType']} — ${record['vaccineName']}',
  ...
  Future<void> deleteVaccinationRecord(String id) async {
    final records =
        getVaccinationRecords().where((r) => r['id'] != id).toList();
    await _prefs?.setString('vaccination_records', jsonEncode(records));
    await deleteTask('vacc_$id');
  }

  // ── Community posts (local cache — CommunityService handles cloud) ──
  ...
  Future<void> addCommunityPost(Map<String, dynamic> post) async {
    ...
    await _prefs?.setString(
        'community_posts', jsonEncode(posts.take(100).toList()));
  }
  ...
  Future<void> addReplyToPost(
      String postId, Map<String, dynamic> reply) async {
...
// ═══════════════════════════════════════════════════════════════
// CommunityService — shared posts via JSONBin
// ═══════════════════════════════════════════════════════════════
class CommunityService {
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        final posts = raw
            ...
            .toList();
        posts.sort((a, b) => (b['time'] as String? ?? '')
  ...
  Future<void> addPost(Map<String, dynamic> post) async {
    ...
    posts.insert(0, post);
    final trimmed =
        posts.length > 200 ? posts.sublist(0, 200) : posts;
  ...
  Future<Map<String, dynamic>?> checkForUpdate() async {
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        final message =
            data['message'] as String? ?? 'FarmConnect $latest is available!';
...
for check in [
    'class StorageService',
    '_sbSelect', '_sbUpsert', '_sbDelete', '_sbUpdate',
    '_kSupabaseUrl', '_kSupabaseKey',
    'savePassword', 'getSavedPhone', 'fetchAndCacheCounties',
    'class CommunityService',
    'class UpdateService',
]:
